...
instalar_whatsmeow() {
  ...
  banner
  printf "${WHITE} >> Digite o TOKEN de autorização do GitHub para acesso ao repositório multiflow:${WHITE}\n"
  echo
  read -p "> " TOKEN_AUTH
  ...
  REPO_URL="https://${TOKEN_AUTH}@github.com/raphaelbat/multiflow-pro.git"
  ...
  if git clone --depth 1 "${REPO_URL}" "${TEST_DIR}" >/dev/null 2>&1; then
    ...
    echo
    printf "${RED}   O teste de git clone falhou. O token informado não tem acesso ao repositório multiflow.${WHITE}\n"
    echo
    ...
    printf "${WHITE}   • https://wa.me/558199988876${WHITE}\n"
    printf "${WHITE}   • https://wa.me/558199988876${WHITE}\n"
    echo
...
# Scripts Equipechat - (81) 9 9998-8876
NODE_ENV=
